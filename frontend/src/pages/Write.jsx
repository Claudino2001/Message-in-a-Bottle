...
function Write() {
  ...
  const validateField = (field, value) => {
    let errorMsg = "";
    if (!value.trim()) errorMsg = "Obrigatório.";
      ...
      <div className="bg-[#fffdf0] p-8 md:p-12 rounded-sm shadow-xl z-10 w-full max-w-2xl relative border border-orange-100 min-h-[400px] flex flex-col justify-center">
        ...
        {hasSentToday ? (
           <div className="text-center">
              <h2 className="text-3xl text-blue-600 font-bold mb-4">🌊 Garrafa lançada!</h2>
              <p className="text-lg text-gray-700">Você já lançou a sua garrafa diária ao oceano.</p>
              <button onClick={() => navigate("/")} className="mt-8 text-blue-500 hover:underline text-lg">Voltar ao início</button>
           </div>
        ) : success ? ( 
          <div className="text-center animate-bounce-slow">
              <h2 className="text-4xl mb-4">🍾</h2>
              <h2 className="text-3xl text-orange-800 font-bold mb-4">Mensagem Enviada!</h2>
              <p className="text-lg text-gray-700">Sua garrafa está flutuando no oceano digital.</p>
              <button onClick={() => navigate("/")} className="mt-8 bg-blue-500 text-white px-6 py-2 rounded-full text-lg">Voltar</button>
          </div>
        ) : (
          <>
            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 w-32 h-8 bg-yellow-200 opacity-60 rotate-2"></div>
            <h2 className="text-3xl text-orange-800 font-bold mb-6 text-center">Escreva sua Mensagem</h2>
